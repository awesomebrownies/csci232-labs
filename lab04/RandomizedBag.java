...
public class RandomizedBag<Item> implements Iterable<Item> {
    ...
    public Item get() {
        ...
        int random = rng.nextInt(a.length);
        Item item = a[random];
        a[random] = a[n];
        a[n] = null;
        return item;
    ...
    private class RandomizedBagIterator implements Iterator<Item> {
        ...
        public RandomizedBagIterator() {
            ...
            */
            //copy over after initializing a new array
            Item[] arrCopy = (Item[]) new Object[a.length];
            for(int i = 0; i < n; i++){
                arrCopy[i] = a[i];
            }
            itArr = arrCopy;
        ...
        public Item next() {
            ...
            //return and remove the random's element
            //take the last element in the array and put it in the empty spot
            Item item = itArr[random];
            itArr[random] = itArr[i-1];
            //decrement i, and don't leave unnecessary memory
            itArr[--i] = null;
            return item;
        }
    }
